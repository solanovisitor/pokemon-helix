...
static const u8 *const sLabPages[][2] = {
    ...
    LAB_PAIR("Same culture. Same medium.\nCompare shelter with exposure.\pWe simulate three small changes.\nPredict the indicator's response.$", "Mesma cultura. Mesmo meio.\nCompare abrigo e area exposta.\pSimulamos tres pequenas mudancas.\nPreveja a resposta do indicador.$"),
    ...
    LAB_PAIR("Calculating the simulated signal.\nB stops without using the medium.$", "Calculando o sinal simulado.\nB para sem usar o meio.$"),
    LAB_PAIR("No result. The medium stays.\nThe reader is offline.\pReturn later to try this assay.$", "Sem resultado. O meio continua.\nLeitor sem conexao.\pVolte depois para tentar o ensaio.$"),
